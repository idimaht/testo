...
app.post('/api/user/login', (req, res) => {
    ...
    client.query(/* ใส่ query string ที่แสดงค่า user ที่มี username=$1จาก table account */'SELECT * FROM user WHERE username=$1', [username], (err, data) => {
        if (err) {
            res.status(401).send('404')
        }
        else if (data.rows[0] == undefined) {
            res.send.status(401).send(404)
        }
        else {
...
app.post('/api/posts', (req, res) => {
    ...
    client.query(/*  เพิ่มค่า(username,topic,content) ลง table post ต่อจาก VALUES ให้ใส่ ($1,$2,$3) RETURNING id ไปเลย*/'INSERT INTO account(username,password) VALUES ($1,$2,$3) RETURNING id', [name, topic, content],
        (err) => {
            if (err) {
                console.log(err.stack)
            }else{
...
app.get('/api/posts/:id', (req, res) => {
    ...
    client.query('SELECT * FROM comment WHERE post_id=$1', [id], (err, data) => {
        if (err) {
            ...
            res.json(allcomment)//ส่ง allcomment กลับไป
        }
    })
})
